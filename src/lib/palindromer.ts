...
export class Palindromer {
  ...
  loadDictionary(words: string[]): void {
    this.dictionary = words.map(word => word.trim().toUpperCase()).filter(word => word.length > 0);
   ...
   */
  generatePalindromes(input: string, options: SearchOptions): PalindromeResult[] {
  ...
  private monteCarloSearch(
    ...
    const selectedExtensions = this.shuffleArray(extensions).slice(0, numToExplore);
    
    for (const extension of selectedExtensions) {
      this.monteCarloSearch(extension.left, extension.right, options, results, depth + 1);
    }
  }
  ...
  private extendOutward(
    ...
    for (const extension of extensions) {
      this.bruteForceSearch(extension.left, extension.right, options, results, depth);
    }
  }
   ...
   */
  private findPossibleExtensions(left: string, right: string): Array<{ left: string; right: string }> {
   ...
   */
  getDictionaryStats(): { wordCount: number; avgLength: number; maxLength: number } {
    if (this.dictionary.length === 0) {
      return { wordCount: 0, avgLength: 0, maxLength: 0 };
    }
    
    const totalLength = this.dictionary.reduce((sum, word) => sum + word.length, 0);
